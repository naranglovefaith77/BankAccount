...
public class Account {
	...
	public Account(int accountNo, String accountType,
			@NotBlank(message = "Please enter Account Holder name") @Pattern(regexp = "^[A-z][a-z]{3,15}", message = "length must be in between 3 to 15") String accountHolderName,
			@NotBlank(message = "Please enter Bank Name") String bankName,
			@NotBlank(message = "Please enter Branch Name") String branchName,
			@NotBlank(message = "Mobile Number is required") @Pattern(regexp = "^[7-9][0-9]{9}$", message = " Enter 10 digit mobile number") long citizenPhoneNumber) {
		super();
